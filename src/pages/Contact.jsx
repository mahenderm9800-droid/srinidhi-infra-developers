...
import { addEnquiry, getProjects, getSettings } from '../services/db';
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitStatus({ 
        type: "success", 
        text: "Your message has been received! Our Sales Representative will reach out within 24 hours." 
      });
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <section className="bg-gradient-to-r from-emerald-800 to-emerald-950 pt-32 pb-12 text-white relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center md:text-left">
          <span className="text-accent-400 text-xs font-semibold tracking-widest uppercase block mb-2">Connect With Us</span>
          <h1 className="text-3xl sm:text-4xl font-serif font-bold text-white">Contact Srinidhi</h1>
          <p className="text-xs sm:text-sm text-slate-450 mt-2 max-w-xl">
            Have questions about our current projects, customized layouts, or legal approvals? Send us a message or visit our corporate office.
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-xl border border-slate-200 shadow-sm space-y-6">
              ...
              <ul className="space-y-5 text-xs text-slate-600 font-medium">
                ...
                <li className="flex items-center">
                  ...
                  <a href={`tel:${settings?.contact?.phone}`} className="hover:text-accent-600 transition-colors">
                    {settings?.contact?.phone || "Loading phone..."}
                  </a>
                </li>
                <li className="flex items-center">
                  <Mail className="h-4.5 w-4.5 mr-3.5 text-accent-500" />
                  <a href={`mailto:${settings?.contact?.email}`} className="hover:text-accent-600 transition-colors">
                ...
                <li className="flex items-start">
                  ...
                  <div>
                    ...
                    <p>{settings?.contact?.hours || "Loading hours..."}</p>
              ...
              <div className="border-t border-slate-150 pt-6">
                <a
                  href={`https://wa.me/${settings?.contact?.whatsapp?.replace(/[^0-9]/g, '') || "919876543210"}?text=Hi,%20I'm%20interested%20in%20Srinidhi%20projects.`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-full inline-flex items-center justify-center px-4 py-3 bg-green-600 hover:bg-green-500 text-white font-bold text-xs rounded-lg transition-colors shadow-md"
                >
            ...
            {/* Quick Notes */}
            <div className="bg-gradient-to-r from-emerald-850 to-emerald-950 text-white p-6 rounded-xl border border-slate-800 shadow-md">
              <h3 className="font-serif text-sm font-bold mb-2">Legal Compliance</h3>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                All Srinidhi projects carry registered state RERA approvals and clear legal titles. We will provide all document sets during your sales verification process.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-6 sm:p-10 rounded-xl border border-slate-205 shadow-sm space-y-6">
              <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6 text-xs font-medium">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="name" className="block text-slate-650 mb-1.5 font-bold uppercase tracking-wider">FullName *</label>
                    <input
                      type="text"
                      id="name"
                      name="name"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label htmlFor="projectRef" className="block text-slate-650 mb-1.5 font-bold uppercase tracking-wider">Project of Interest</label>
                    <select
                      id="projectRef"
                      name="projectRef"
                      value={formData.projectRef}
                      onChange={handleInputChange}
                      className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-accent-500 text-slate-850 bg-white"
                    >
                ...
                <div>
                  <label htmlFor="message" className="block text-slate-650 mb-1.5 font-bold uppercase tracking-wider">Message / Details</label>
                  <textarea
                    id="message"
                    name="message"
                    rows="5"
                    ...
                    className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-accent-500 text-slate-800 resize-none font-sans"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center justify-center px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg transition-colors disabled:opacity-50 text-xs uppercase tracking-wider shadow"
                >
                  {submitting ? "Sending..." : (
                    <>
